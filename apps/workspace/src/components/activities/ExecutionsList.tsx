...
export default function ExecutionsList({ executions, agents, title = 'Executions', formatDuration, onSelect, page, pageSize, onPageChange }: ExecutionsListProps) {
    ...
    return (
      <div className="bg-(--vestara-accent-bg) border border-(--vestara-accent-border) rounded-lg p-5">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-xs font-semibold text-(--vestara-text-muted) uppercase tracking-wider">{title}</h3>
        </div>
  ...
  return (
    <div className="bg-(--vestara-accent-bg) border border-(--vestara-accent-border) rounded-lg p-4 flex flex-col min-h-0">
      <div className="flex items-center justify-between mb-3 shrink-0">
        <h3 className="text-xs font-semibold text-(--vestara-text-muted) uppercase tracking-wider">{title} ({executions.length})</h3>
      </div>
            ...
            {display.map((exec, i) => {
              ...
              const duration = exec.completedAt
                ...
                : exec.status === 'running'
                  ...
                  : null;
              return (
                <tr key={exec.id || i} onClick={() => onSelect?.(exec)} className="border-b border-(--vestara-accent-border)/50 hover:bg-(--vestara-accent-bg) cursor-pointer transition-colors">
