...
export default function SettingsRouter({ modules, routes, fallback }: SettingsRouterProps) {
  ...
  if (!route) {
    return <>{fallback || <div className="text-(--vestara-text-2)">Select a setting from the sidebar.</div>}</>;
  }
  ...
  if (!module) {
    return <div className="text-(--vestara-red)">Module not found: {route.moduleId}</div>;
  }
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-(--vestara-text) mb-4">{module.name}</h1>
      {module.description && <p className="text-(--vestara-text-2) mb-6">{module.description}</p>}
      <div className="bg-(--vestara-accent-bg) rounded-lg p-6 border border-(--vestara-accent-border)">
        <p className="text-(--vestara-text-2)">
