 ...
 *
 * Each panel below is a standalone route element (/settings/profiles,
 * /settings/appearance, /settings/typography, /settings/layout) sharing the
 * single useTheme authority. AppearanceControls keeps the legacy combined
 * tabbed surface (used only where embedding is still required); new
 * navigation renders the panels directly with no duplicated forms or state.
 */
function SelectField<K extends keyof ThemeSettings>({
  settingKey,
  options,
}: {
  settingKey: K;
...
/** Standalone route element for /settings/profiles. */
    ...
    >
      <div className="grid gap-2 p-4 sm:grid-cols-2 xl:grid-cols-4">
        {PROFILES.map((profile) => (
          ...
          >
            <span className="font-mono text-[10px] text-[var(--vestara-accent-text)]">{profile.id.toUpperCase()}</span>
...
/** Standalone route element for /settings/appearance. */
export function AppearancePanel() {
  const { mode, resolved, settings, setMode, updateSetting } = useTheme();
  return (
...
/** Standalone route element for /settings/typography. */
export function TypographyPanel() {
  return (
    <SettingsSection title="Typography" description="Runtime font variables update every Workspace surface.">
      <SettingsRow
        label="Font family"
        value={<SelectField settingKey="fontFamily" options={['system', 'serif', 'mono'] as const} />}
      />
      <SettingsRow
        label="Font size"
        value={<SelectField settingKey="fontSize" options={['small', 'medium', 'large'] as const} />}
      />
      <SettingsRow
        label="Font weight"
        value={<SelectField settingKey="fontWeight" options={['normal', 'medium', 'semibold'] as const} />}
      />
    </SettingsSection>
  );
}

/** Standalone route element for /settings/layout. */
export function LayoutPanel() {
  ...
  return (
    ...
    >
      <SettingsRow
        label="Sidebar width"
        value={<SelectField settingKey="sidebarWidth" options={['compact', 'normal', 'wide'] as const} />}
      />
      <SettingsRow
        label="Sidebar mode"
        value={
