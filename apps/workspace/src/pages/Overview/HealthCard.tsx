...
function barColor(score: number): string {
  if (score >= 7) return 'bg-(--vestara-green)';
...
export function HealthCard({ data }: { data: UnderstandingData }) {
  ...
  return (
    <div className="bg-(--vestara-accent-bg) border border-(--vestara-accent-border) rounded-lg p-5">
      <h2 className="text-lg font-semibold text-(--vestara-text) mb-3">Health</h2>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="text-sm text-(--vestara-text-2)">
          <div className="capitalize font-medium text-(--vestara-text)">{h.level}</div>/ 10
        </div>
      </div>
      ...
      <div className="space-y-2 text-sm">
        <div className="flex justify-between">
          <span className="text-(--vestara-text-2)">Test Coverage</span>
          <span className="font-medium text-(--vestara-text) capitalize">{h.testCoverage}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-(--vestara-text-2)">Code Quality</span>
          <span className="font-medium text-(--vestara-text) capitalize">{h.codeQuality}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-(--vestara-text-2)">Documentation</span>
          <span className="font-medium text-(--vestara-text) capitalize">{h.documentationLevel}</span>
        </div>
      </div>
          ...
          <div className="space-y-1">
            {h.risks.slice(0, 3).map((r, i) => (
              <div key={i} className="text-sm flex gap-2">
                ...
                <span className="text-(--vestara-text-2) truncate">{r.summary}</span>
              </div>
            ))}
          </div>
        </div>
