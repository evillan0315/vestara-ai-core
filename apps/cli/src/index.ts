...
function registerCommands(registry: CommandRegistry): void {
  ...
  registry.register('teams', (args) => {
    if (args[0] === 'create' && args[1]) return runTeamsCreate(args.slice(1));
    if (args[0] === 'assign' && args[1] && args[2]) return runTeamsAssign(args[1], args.slice(2));
    return runTeamsList();
  });
  ...
  registry.register('help', (args) => { if (args[0]) runHelpCommand(args[0]); else printHelp(); });
  registry.register('completions', (args) => runCompletions(args[0]));
  registry.register('projects', () => runProjectsList());
  registry.register('session', (args) => {
    if (args[0] === 'workflows') return runListWorkflows();
    if (args[0] === 'start' && args[1] && args[2]) return runStartSession(args[1], args[2]);
    if (args[0] === 'list') return runListSessions();
    if (args[0] === 'background') return runBackgroundServices();
    console.log(`${GOLD}Usage: vestara session workflows|start|list|background${RESET}`);
  });
  registry.register('doctor', (args) => {
    if (args[0] === 'audio') return runDoctorAudio();
    if (args[0] === 'conversation') return runDoctorConversation();
    if (args[0] === 'agents') return runDoctorAgents();
    if (args[0] === 'teams') return runDoctorTeams();
    if (args[0] === 'models') return runDoctorModels();
    if (args[0] === 'workspace') return runDoctorWorkspace();
    if (args[0] === 'all') return runDoctorAll();
    return runDoctor();
  });
  registry.register('benchmark', (args) => {
    if (args[0] === 'conversation') return runBenchmarkConversation();
    console.log(`${GOLD}Usage: vestara benchmark conversation${RESET}`);
  });
  registry.register('demo', (args) => {
    if (args[0] === 'golden-path') return runGoldenPath();
...
async function main() {
  ...
  if (args[0] === 'config') {
    if (args[1] === 'list') { const { runConfigGet } = await import('./commands/config.js'); await runConfigGet(undefined); return; }
    if (args[1] === 'get') { const { runConfigGet } = await import('./commands/config.js'); await runConfigGet(args[2]); return; }
  ...
  if (args[0] === 'provider') {
    ...
    if (args[1] === 'status' && args[2]) { const { runProviderStatus } = await import('./commands/provider.js'); await runProviderStatus(args[2]); return; }
    console.log(`${GOLD}Usage:${RESET}`);
  ...
  if (args[0] === 'task') {
    ...
    if (args[1] === 'list' && args[2]) { const { runTaskList } = await import('./commands/task.js'); await runTaskList(args[2]); return; }
    console.log(`${GOLD}Usage:${RESET}`);
...
import { renderStatus } from './output/format.js';
...
async function startBasicRepl(): Promise<void> {
  const readline = (await import('node:readline')).default;
