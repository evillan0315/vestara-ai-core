...
import { BOLD, GOLD, GREEN, RED, GRAY, RESET, renderStatus } from './output/format.js';
...
import { runTeamsList, runTeamsCreate, runTeamsAssign } from './commands/teams.js';
import { runPlansList, runPlanShow, runPlanApprove, runPlanDelete, runPlanCreate, runPlanUpdateStatus } from './commands/plans.js';
import { runContext } from './commands/context.js';
import { runMetrics } from './commands/metrics.js';
import { runModelsList } from './commands/models.js';
import { runListWorkflows, runStartSession, runListSessions, runBackgroundServices } from './commands/session.js';
import { runCompletions } from './commands/completions.js';
import { runHelpCommand } from './commands/help-cmd.js';
import { runDoctor, runDoctorAudio, runDoctorConversation, runDoctorAgents, runDoctorTeams, runDoctorModels, runDoctorWorkspace, runDoctorAll } from './commands/doctor.js';
...
function printVersion(): void { console.log(`vestara v${VERSION}`); }
...
function printHelp(): void {
  ...
  console.log(`    ${GREEN}open${RESET} [path]        ${GRAY}Open a workspace (default: ., --force to re-open)${RESET}`);
  ...
  console.log(`    ${GREEN}doctor${RESET} [sub]       ${GRAY}Diagnostics (audio|conversation|agents|teams|models|workspace|all)${RESET}`);
  ...
  console.log(`    ${GREEN}session${RESET} <sub>      ${GRAY}Session management (workflows|start|list|background)${RESET}`);
  ...
  console.log(`    ${GREEN}provider${RESET} [sub]     ${GRAY}Provider management (add|list|status|enable|disable|remove|models)${RESET}`);
  console.log(`    ${GREEN}plan${RESET} [sub]         ${GRAY}Plan management (list|show|create|approve|status|delete)${RESET}`);
  ...
  console.log(`    ${GREEN}architecture${RESET} [sub]  ${GRAY}AKG query (show|list|depends-on|dependents-of|influences|impact)${RESET}`);
  console.log(`    ${GREEN}blueprint${RESET} verify    ${GRAY}Architecture Knowledge Graph integrity check${RESET}`);
  console.log(`    ${GREEN}ops${RESET} [sub]           ${GRAY}Engineering telemetry (feed|status|timeline|demo)${RESET}`);
...
function registerCommands(registry: CommandRegistry): void {
  ...
  registry.register('teams', async (args) => {
    if (args[0] === 'create' && args[1]) { await runTeamsCreate(args.slice(1)); return; }
    if (args[0] === 'assign' && args[1] && args[2]) { await runTeamsAssign(args[1], args.slice(2)); return; }
    await runTeamsList();
  });
  ...
  registry.register('help', async (args) => { if (args[0]) await runHelpCommand(args[0]); else printHelp(); });
  ...
  registry.register('session', async (args) => {
    if (args[0] === 'workflows') { await runListWorkflows(); return; }
    if (args[0] === 'start' && args[1] && args[2]) { await runStartSession(args[1], args[2]); return; }
    if (args[0] === 'list') { await runListSessions(); return; }
    if (args[0] === 'background') { await runBackgroundServices(); return; }
    console.log(`${GOLD}Usage: vestara session workflows|start|list|background${RESET}`);
  });
  registry.register('doctor', async (args) => {
    if (args[0] === 'audio') { await runDoctorAudio(); return; }
    if (args[0] === 'conversation') { await runDoctorConversation(); return; }
    if (args[0] === 'agents') { await runDoctorAgents(); return; }
    if (args[0] === 'teams') { await runDoctorTeams(); return; }
    if (args[0] === 'models') { await runDoctorModels(); return; }
    if (args[0] === 'workspace') { await runDoctorWorkspace(); return; }
    if (args[0] === 'all') { await runDoctorAll(); return; }
    await runDoctor();
  });
  registry.register('benchmark', async (args) => {
    if (args[0] === 'conversation') { await runBenchmarkConversation(); return; }
    console.log(`${GOLD}Usage: vestara benchmark conversation${RESET}`);
  });
  registry.register('demo', async (args) => {
    if (args[0] === 'golden-path') { await runGoldenPath(); return; }
...
export async function main() {
  ...
  if (args[0] === '--help' || args[0] === '-h') { printHelp(); return; }
  if (args[0] === 'help') { if (args[1]) { await runHelpCommand(args[1]); return; } printHelp(); return; }
  if (args[0] === '--version' || args[0] === '-v') { printVersion(); return; }

  if (args[0] === 'status') { await runSystemStatus(args.slice(1)); return; }

  if (args[0] === 'agents') { await runAgentsList(); return; }

  if (args[0] === 'teams') {
    if (args[1] === 'create' && args[2]) { await runTeamsCreate(args.slice(2)); return; }
    if (args[1] === 'assign' && args[2] && args[3]) { await runTeamsAssign(args[2], args.slice(3)); return; }
    await runTeamsList(); return;
  }

  if (args[0] === 'metrics') { await runMetrics(); return; }

  if (args[0] === 'doctor') {
    if (args[1] === 'audio') { await runDoctorAudio(); return; }
    if (args[1] === 'conversation') { await runDoctorConversation(); return; }
    if (args[1] === 'agents') { await runDoctorAgents(); return; }
    if (args[1] === 'teams') { await runDoctorTeams(); return; }
    if (args[1] === 'models') { await runDoctorModels(); return; }
    if (args[1] === 'workspace') { await runDoctorWorkspace(); return; }
    if (args[1] === 'all') { await runDoctorAll(); return; }
    await runDoctor(); return;
  }

  if (args[0] === 'benchmark') {
    if (args[1] === 'conversation') { await runBenchmarkConversation(); return; }
    console.log(`${GOLD}Usage: vestara benchmark conversation${RESET}`); return;
  }

  if (args[0] === 'demo') {
    if (args[1] === 'golden-path') { await runGoldenPath(); return; }
    console.log(`${GOLD}Usage: vestara demo golden-path${RESET}`); return;
  }

  if (args[0] === 'session') {
    if (args[1] === 'workflows') { await runListWorkflows(); return; }
    if (args[1] === 'start' && args[2] && args[3]) { await runStartSession(args[2], args[3]); return; }
    if (args[1] === 'list') { await runListSessions(); return; }
    if (args[1] === 'background') { await runBackgroundServices(); return; }
  ...
  if (args[0] === 'config') {
    if (args[1] === 'list' || args[1] === 'get') { console.log(`${GRAY}Use 'vestara status' or 'vestara context' to view configuration.${RESET}\n`); return; }
    if (args[1] === 'set' && args[2] && args[3]) { const { runConfigSet } = await import('./commands/config.js'); await runConfigSet(args[2], args.slice(3).join(' ')); return; }
    if (args[1] === 'reset' && args[2]) { const { runConfigReset } = await import('./commands/config.js'); await runConfigReset(args[2]); return; }
    console.log(`${GRAY}Usage: vestara config set <key> <value> | vestara config reset <key>${RESET}\n`);
    return;
  }

  if (args[0] === 'plans') { await runPlansList(args.slice(1)); return; }

  if (args[0] === 'models') { await runModelsList(); return; }

  if (args[0] === 'provider') {
    const { runProviderAdd, runProviderAddLocal, runProviderRemove, runProviderEnable, runProviderDisable, runProviderEnhancedList, runProviderModelsList, runProviderModelAdd, runProviderModelEnable, runProviderModelDisable } = await import('./commands/provider.js');
    if (args[1] === 'add') { await runProviderAdd(args.slice(2)); return; }
    if (args[1] === 'add-local') { await runProviderAddLocal(args.slice(2)); return; }
    if (args[1] === 'remove' && args[2]) { await runProviderRemove(args[2]); return; }
    if (args[1] === 'enable' && args[2]) { await runProviderEnable(args[2]); return; }
    if (args[1] === 'disable' && args[2]) { await runProviderDisable(args[2]); return; }
    if (args[1] === 'list') { await runProviderEnhancedList(); return; }
    if (args[1] === 'models' && args[2]) { await runProviderModelsList(args[2]); return; }
    if (args[1] === 'model' && args[2] === 'add' && args[3]) { await runProviderModelAdd(args[3], args[4] ?? '', args.slice(5)); return; }
    if (args[1] === 'model' && args[2] === 'enable' && args[3] && args[4]) { await runProviderModelEnable(args[3], args[4]); return; }
    if (args[1] === 'model' && args[2] === 'disable' && args[3] && args[4]) { await runProviderModelDisable(args[3], args[4]); return; }
    if (args[1] === 'status' && args[2]) { console.log(`${GRAY}Checking provider status...${RESET}`); return; }
    console.log(`${GOLD}Usage:${RESET}`);
  ...
  if (args[0] === 'projects') { await runProjectsList(); return; }

  if (args[0] === 'completions') { await runCompletions(args[1]); return; }

  if (args[0] === 'plan' || args[0] === 'plans') {
    if (args[1] === 'show' && args[2]) { await runPlanShow(args[2]); return; }
    if (args[1] === 'approve' && args[2]) { await runPlanApprove(args[2]); return; }
    if (args[1] === 'delete' && args[2]) { await runPlanDelete(args[2]); return; }
    if (args[1] === 'create' && args[2]) { await runPlanCreate(args.slice(2).join(' ')); return; }
    if (args[1] === 'status' && args[2] && args[3]) { await runPlanUpdateStatus(args[2], args[3]); return; }
    await runPlansList(args.slice(1));
    return;
  }

  if (args[0] === 'task') {
    if (args[1] === 'create' && args[2]) { const { runTaskCreate } = await import('./commands/task.js'); await runTaskCreate(args[2]); return; }
    if (args[1] === 'assign' && args[2] && args[3]) { const { runTaskAssign } = await import('./commands/task.js'); await runTaskAssign(args[2], args[3]); return; }
    if (args[1] === 'prioritize' && args[2] && args[3]) { const { runTaskPrioritize } = await import('./commands/task.js'); await runTaskPrioritize(args[2], args[3]); return; }
    if (args[1] === 'comment' && args[2] && args[3]) { const { runTaskComment } = await import('./commands/task.js'); await runTaskComment(args[2], args.slice(3).join(' ')); return; }
    if (args[1] === 'list' && args[2]) { const { runPlanShow } = await import('./commands/plans.js'); await runPlanShow(args[2]); return; }
    console.log(`${GOLD}Usage:${RESET}`);
  ...
  if (args[0] === 'context') { await runContext(); return; }

  if (args[0] === 'architecture') { await runArchitecture(args.slice(1)); return; }

  if (args[0] === 'blueprint' && args[1] === 'verify') { await runBlueprintVerify(); return; }

  if (args[0] === 'ops') { await runOps(args.slice(1)); return; }
  ...
  const { kernel, stateRuntime, conversationService: routedConversationService, conversationEngine, conversationId, audioService, sttService, ttsService, workspaceRuntime, workspaceHealth } = ctx;
  ...
  try {
    ...
    if (profile.frameworks.length > 0) console.log(`  ${GRAY}Framework: ${profile.frameworks.map((f: any) => f.name).join(', ')}${RESET}`);
    if (profile.packageManager) console.log(`  ${GRAY}Package Manager: ${profile.packageManager.name}${RESET}`);
    if (workspaceHealth.isGitRepository) console.log(`  ${GRAY}Git: ${profile.identity.gitBranch ?? 'detected'}${RESET}`);
  } catch {}
  ...
  if (healthStatus.health.overall === 'healthy') console.log(`${GREEN}Runtime Healthy${RESET}`);
    ...
    for (const s of healthStatus.services) { if (s.health !== 'healthy') console.log(`  ${GOLD}⚠${RESET} ${s.id}: ${s.health}`); }
  } else {
  ...
  console.log(`${BOLD}Vestara${RESET}: ${greeting}`); console.log();

  const audioStatus = await audioService.diagnose();
  const audioIcon = audioStatus.microphone.available ? `${GREEN}✓${RESET}` : `${GRAY}○${RESET}`;
  console.log(`  ${GRAY}Audio: ${audioIcon} Mic${RESET}${audioStatus.vad.status !== 'idle' ? ` ${GREEN}✓${RESET} VAD` : ''}${audioStatus.stt.available ? ` ${GREEN}✓${RESET} STT` : ''}${audioStatus.tts.available ? ` ${GREEN}✓${RESET} TTS` : ''}`);
  console.log();
  console.log(`${GRAY}Ready.${RESET}`); console.log();
...
export async function startRepl(ctx: CliContext): Promise<void> {
  ...
  rl.on('line', async (line: string) => {
    ...
    if (input === 'exit' || input === 'quit') { await ctx.close(); rl.close(); return; }
    if (input === 'health' || input === 'status') {
      const diag: any = await kernel.diagnose();
      console.log(`  Status: ${diag.status}`); console.log(`  Uptime: ${diag.uptime}s`);
      console.log(`  Health: ${diag.health.overall} (${diag.health.healthyCount}/${diag.health.totalServices} healthy)`);
      console.log(`  Memory: ${diag.resources.memory.heapUsed}MB / ${diag.resources.memory.heapTotal}MB`);
      console.log(`  Workspace: ${ctx.workspaceHealth.indexedFiles} files, ${ctx.workspaceHealth.isGitRepository ? 'git' : 'no git'}`);
      rl.prompt(); return;
    }
    if (input === 'workspace') {
      const p = workspaceRuntime.profile;
      console.log(`  Project: ${BOLD}${p.name}${RESET}`); console.log(`  Root: ${p.identity.rootPath}`); console.log(`  Language: ${p.primaryLanguage.name}`);
    ...
    if (input === 'profile' && conversationEngine) {
      ...
      if (p) {
        console.log(`  Name:              ${p.name ?? '(not set)'}`); console.log(`  Role:              ${p.role ?? '(not set)'}`); console.log(`  Experience:        ${p.experience ?? '(not set)'}`);
        console.log(`  Preferred Stack:   ${(p.preferredStack ?? []).join(', ') || '(not set)'}`); console.log(`  Communication:     ${p.communicationStyle}`);
        console.log(`  Goals:             ${(p.goals ?? []).join(', ') || '(none)'}`); console.log(`  Conversations:     ${p.conversationCount}`); console.log(`  Since:             ${p.createdAt}`);
      } else console.log(`  ${GRAY}No profile found.${RESET}`);
      rl.prompt(); return;
    }
    if (input.startsWith('!')) {
      const cmd = input.slice(1).trim();
      if (!cmd) { console.log(`${GRAY}  Usage: !<shell command>${RESET}`); rl.prompt(); return; }
      try {
      ...
      } catch (error: any) { console.log(error.stdout?.toString()?.trim() || error.stderr?.toString()?.trim() || error.message); }
      rl.prompt(); return;
    }
    if (input === 'help') { console.log('  Commands: health, status, workspace, history, profile, !<cmd>, help, exit, quit'); rl.prompt(); return; }
    if (input === 'history') {
      ...
      if (conversationService && conversationId) {
        ...
        try {
          for await (const chunk of conversationService.sendMessageStream(conversationId, input)) {
            ...
            else if (chunk.type === 'error' && chunk.content) process.stdout.write(`\n${RED}Error: ${chunk.content}${RESET}\n`);
          }
          process.stdout.write('\n\n');
        } catch (err: any) {
          const m = err?.message ?? '';
  ...
  rl.on('close', () => { console.log(); ctx.close().catch(() => {}); process.exit(0); });
}
...
export async function startBasicRepl(): Promise<void> {
  ...
  rl.on('line', (line: string) => {
    ...
    if (input === 'help') { console.log('  Commands: help, exit, quit'); }
    else { console.log(`${GRAY}Runtime not available. Run without --no-repl for full features.${RESET}`); }
    rl.prompt();
  });
