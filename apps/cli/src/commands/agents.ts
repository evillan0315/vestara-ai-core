...
export async function runAgentsList(): Promise<void> {
  ...
  try {
    ...
    const ROLE_COLORS: Record<string, string> = {
      ...
      conversation: '#6366f1', planning: '#eab308', 'dashboard-curator': '#06b6d4',
      frontend: '#ec4899', custom: '#6b7280',
    };
    for (const agent of agents) {
      ...
      const statusIcon = agent.status === 'active' ? `${GREEN}●${RESET}` : agent.status === 'disabled' ? `${GRAY}○${RESET}` : `${RED}○${RESET}`;
      const team = teams.find((t: any) => t.id === agent.teamId);
      console.log(`  ${statusIcon} ${agent.name.padEnd(22)} ${agent.role.padEnd(16)} ${agent.status === 'active' ? `${GREEN}active${RESET}` : `${GRAY}disabled${RESET}`}  ${GRAY}${agentExecs.length} exec, ${successRate}% success${RESET}`);
      if (agent.description) console.log(`  ${' '.repeat(4)}${GRAY}${agent.description}${RESET}`);
      if (agent.provider || agent.model) console.log(`  ${' '.repeat(4)}${GRAY}${agent.provider || ''} ${agent.model || ''}${RESET}`);
    ...
    console.log(`${GRAY}  ${agents.length} agents, ${execs.length} total executions, ${schedules.length} schedules${RESET}`);
    console.log();
  } catch (err: any) {
