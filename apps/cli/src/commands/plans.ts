...
export async function runPlansList(cliArgs?: string[]): Promise<void> {
  ...
  try {
    ...
    try { const raw = fs.readFileSync(manifestPath, 'utf-8'); const manifest = JSON.parse(raw); if (manifest.id) workspaceId = manifest.id; } catch {}
    ...
    if (useJson) {
      console.log(JSON.stringify({ total: plans.length, tasks: totalTasks, statuses: statusCounts, plans: plans.map((p: any) => ({ id: p.id, title: p.title, goal: p.goal, status: p.status, tasks: p.tasks?.length ?? 0, completedTasks: p.tasks?.filter((t: any) => t.status === 'completed').length ?? 0, createdAt: p.createdAt, updatedAt: p.updatedAt })) }, null, 2));
      return;
    }
    ...
    console.log();
    if (plans.length === 0) { console.log(`  ${GRAY}No plans found.${RESET}\n`); return; }

    for (const plan of plans) {
      const statusIcon = plan.status === 'executing' ? `${GREEN}●${RESET}` : plan.status === 'completed' ? `${GREEN}✔${RESET}` : plan.status === 'cancelled' ? `${RED}✘${RESET}` : `${GRAY}○${RESET}`;
      ...
      if (plan.goal && plan.goal !== plan.title) console.log(`       ${plan.goal.length > 100 ? plan.goal.slice(0, 97) + '...' : plan.goal}`);
...
export async function runPlanShow(id: string): Promise<void> {
  ...
  try {
    ...
    if (!plan) { console.log(`${RED}Plan "${id}" not found.${RESET}\n`); return; }
...
export async function runPlanCreate(goal: string): Promise<void> {
  ...
  try {
    ...
    const manifestPath = path.join(wsDir, 'workspace.json');
    let workspaceId = 'default';
    let profile: any = { name: 'workspace', language: 'typescript', isMonorepo: false, packages: [], entryPoints: [], risks: [] };
    try {
      if (fs.existsSync(manifestPath)) {
        const raw = fs.readFileSync(manifestPath, 'utf-8');
        const manifest = JSON.parse(raw);
        workspaceId = manifest.id || workspaceId;
        profile = { name: manifest.name || 'workspace', language: manifest.analysis?.language || 'typescript', isMonorepo: manifest.analysis?.isMonorepo || false, packages: manifest.analysis?.packages || [], entryPoints: manifest.analysis?.entryPoints || [], risks: manifest.analysis?.risks || [] };
      }
    } catch {}
    ...
    const miniSession = { fingerprint: { id: workspaceId }, profile, memory: { search: async () => ({ memories: [] }), store: async () => {} }, storeMemory: async () => {} };
    const result = await planner.createPlan(goal, miniSession as any);
    console.log(`  ${GREEN}✓${RESET} Plan created  ${BOLD}(${result.plan.id})${RESET}  ${GRAY}source: ${result.source}, ${(result.duration / 1000).toFixed(1)}s${RESET}`);
    ...
    for (const ts of result.plan.tasks.map((t: any) => `${t.id}: ${t.summary} (${t.effort})`)) console.log(`    · ${ts}`);
    console.log();
  } catch (err: any) { console.log(`  ${RED}Error: ${err.message}${RESET}\n`); }
}
...
export async function runPlanUpdateStatus(id: string, status: string): Promise<void> {
  ...
  if (!validStatuses.includes(status)) { console.log(`${RED}Invalid status: "${status}"${RESET}\n`); return; }
  console.log();
  try {
    const db = await openSharedDb();
    const { PlanStorage, PlanningService } = await import('@vestara/workspace');
    ...
    console.log(`${GREEN}✓${RESET} Plan ${BOLD}${id.toUpperCase()}${RESET}: ${prevStatus} → ${GREEN}${status}${RESET}  ${GRAY}(${plan.title})${RESET}\n`);
  } catch (err: any) { console.log(`  ${RED}Error: ${err.message}${RESET}\n`); }
}
