...
export async function runDoctor(): Promise<void> {
  ...
  try {
    ...
    console.log(`  Overall Health: ${overallColor}`);
      ...
      const icon =
        svc.health === 'healthy'
          ...
          : svc.health === 'degraded'
            ...
            : `${RED}✗${RESET}`;
      console.log(
        `  ${icon} ${svc.id.padEnd(22)} ${svc.health.padEnd(12)} ${GRAY}${svc.latency > 0 ? `${svc.latency}ms` : '-'}${RESET}`,
      );
    }
    console.log();
...
export async function runDoctorConversation(): Promise<void> {
  ...
  function srcIcon(s: boolean) {
    return s ? `${GREEN}●${RESET}` : `${RED}○${RESET}`;
  }
  function okIcon(s: boolean) {
    return s ? `${GREEN}✔${RESET}` : `${RED}✗${RESET}`;
  }
...
export async function runDoctorAudio(): Promise<void> {
  ...
  console.log(
    `  ${audioStatus.microphone.available ? `${GREEN}✔${RESET}` : `${RED}✗${RESET}`} Microphone  ${audioStatus.microphone.available ? 'Ready' : 'Not found'}  ${GRAY}${audioStatus.microphone.latency}ms${RESET}`,
  );
  console.log(
    `  ${audioStatus.speakers.available ? `${GREEN}✔${RESET}` : `${GRAY}○${RESET}`} Speaker     ${audioStatus.speakers.available ? 'Ready' : 'Not found'}  ${GRAY}${audioStatus.speakers.latency}ms${RESET}`,
  );
  console.log(
    `  ${audioStatus.vad.status !== 'error' ? `${GREEN}✔${RESET}` : `${RED}✗${RESET}`} VAD         ${audioStatus.vad.status !== 'error' ? 'Ready' : 'Error'}  ${GRAY}${audioStatus.vad.latency}ms${RESET}`,
  );
  console.log();
  console.log(`  ${BOLD}Speech-to-Text${RESET}`);
  console.log(
    `  ${sttHealth.status === 'healthy' ? `${GREEN}✔${RESET}` : `${RED}✗${RESET}`} STT         ${sttHealth.status === 'healthy' ? 'Ready' : 'Unavailable'}  ${GRAY}${sttHealth.latency}ms${RESET}`,
  );
  console.log();
  console.log(`  ${BOLD}Text-to-Speech${RESET}`);
  console.log(
    `  ${ttsHealth.status === 'healthy' ? `${GREEN}✔${RESET}` : `${RED}✗${RESET}`} TTS         ${ttsHealth.status === 'healthy' ? 'Ready' : 'Unavailable'}  ${GRAY}${ttsHealth.latency}ms${RESET}`,
  );
  console.log();
}
...
export async function runDoctorAgents(): Promise<void> {
  ...
  try {
    ...
    for (const agent of agents) {
      ...
      console.log(
        `  ${aRunning > 0 ? `${GOLD}⚡${RESET}` : `${GREEN}●${RESET}`} ${agent.name.padEnd(22)} ${agent.status === 'active' ? `${GREEN}active${RESET}` : `${GRAY}disabled${RESET}`}  ${aTotal > 0 ? `${aCompleted}/${aTotal} (${aRate}%)` : 'no executions'}${aFailed > 0 ? `  ${RED}${aFailed} failed${RESET}` : ''}`,
      );
    }
    console.log();
    const progress = ms.getProgress();
  ...
  try {
    ...
    if (teams.length > 0) {
      ...
      for (const team of teams) {
        ...
        console.log(
          `  ${leader ? `${GREEN}●${RESET}` : `${GOLD}⚠${RESET}`} ${team.name.padEnd(22)} ${members.length} members, ${totalExecs} exec, ${failedExecs} failed${runningExecs > 0 ? `, ${runningExecs} running` : ''}`,
        );
        console.log(
        ...
        else console.log(`  ${' '.repeat(4)}${GOLD}⚠ No leader assigned${RESET}`);
        console.log();
      }
    }
...
export async function runDoctorModels(): Promise<void> {
  console.log();
  console.log(`${BOLD}${GOLD}Provider & Model Diagnostics${RESET}`);
  console.log(`${GRAY}─────────────────────────────────────${RESET}`);
  console.log();
  try {
    const { OpenCodeProvider } = await import('@vestara/provider-opencode');
    ...
    const healthIcon =
      health.status === 'healthy'
        ...
        : health.status === 'degraded'
          ? `${GOLD}●${RESET}`
          : `${RED}●${RESET}`;
    console.log(`  ${BOLD}OpenCode Provider${RESET}`);
    console.log(`  ${healthIcon} Status:     ${health.status}`);
    console.log(`    Latency:    ${health.latency}ms`);
    console.log(`    Heartbeat:  ${health.lastHeartbeat ? new Date(health.lastHeartbeat).toLocaleString() : 'never'}`);
    ...
    for (const m of models) {
      ...
      if (m.capabilities.chat) features.push('chat');
      if (m.capabilities.streaming) features.push('streaming');
      if (m.capabilities.functionCalling) features.push('function-calling');
      if (m.capabilities.vision) features.push('vision');
      const pricing =
        m.pricing && m.pricing.inputPerMillionTokens === 0 && m.pricing.outputPerMillionTokens === 0
          ? 'free'
          : `$${m.pricing?.inputPerMillionTokens ?? '?'} in / $${m.pricing?.outputPerMillionTokens ?? '?'} out`;
      console.log(`  ${GREEN}✓${RESET} ${m.name}  ${GRAY}(${m.id})${RESET}`);
      console.log(
        `       Context: ${(m.contextWindow / 1000).toFixed(0)}K  ·  Output: ${(m.maxOutput / 1000).toFixed(0)}K  ·  ${pricing}`,
      );
    ...
    if (fs.existsSync(prefsPath)) {
      try {
        ...
        const buf = fs.readFileSync(prefsPath);
        const db = new SQL.Database(buf);
        const rows = db.exec("SELECT value FROM preferences WHERE key = 'model'");
        const prow = db.exec("SELECT value FROM preferences WHERE key = 'provider'");
        const currentModel =
          rows && rows.length > 0 && rows[0].values.length > 0 ? String(rows[0].values[0][0] ?? '') : 'default';
        const currentProvider =
          prow && prow.length > 0 && prow[0].values.length > 0 ? String(prow[0].values[0][0] ?? '') : 'default';
        console.log(`  ${BOLD}Active Configuration${RESET}`);
        console.log(`    Provider:   ${currentProvider}`);
        console.log(`    Model:      ${currentModel}`);
        console.log(`    Change:     vestara config set model <model-id>`);
        console.log();
        db.close();
      } catch {}
    }
  } catch (err: any) {
    console.log(`  ${RED}Diagnostic error: ${err.message}${RESET}\n`);
    process.exitCode = 1;
  }
}
...
export async function runDoctorWorkspace(): Promise<void> {
  console.log();
  console.log(`${BOLD}${GOLD}Workspace Diagnostics${RESET}`);
  console.log(`${GRAY}─────────────────────────────────────${RESET}`);
  console.log();
  const wsDir = path.join(process.cwd(), '.vestara');
  const checks: { label: string; ok: boolean; detail?: string }[] = [];
  const hasDir = fs.existsSync(wsDir);
  checks.push({ label: '.vestara directory', ok: hasDir, detail: hasDir ? wsDir : 'not found' });
  const manifestPath = path.join(wsDir, 'workspace.json');
  const hasManifest = fs.existsSync(manifestPath);
  checks.push({ label: 'workspace.json', ok: hasManifest });
  let manifest: any = null;
  if (hasManifest) {
    try {
      const raw = fs.readFileSync(manifestPath, 'utf-8');
      manifest = JSON.parse(raw);
      checks.push({ label: 'Manifest valid JSON', ok: true });
    } catch {
      checks.push({ label: 'Manifest valid JSON', ok: false, detail: 'parse error' });
    }
  }
  ...
  for (const check of checks) {
    console.log(
      `  ${check.ok ? `${GREEN}✓${RESET}` : `${RED}✗${RESET}`} ${check.label.padEnd(22)} ${check.detail ? GRAY + check.detail + RESET : ''}`,
    );
    if (!check.ok) allOk = false;
  }
  console.log();
  if (manifest) {
    console.log(`  ${BOLD}Workspace Summary${RESET}`);
    console.log(`    Name:       ${manifest.name || '?'}`);
    console.log(`    ID:         ${manifest.id || '?'}`);
    console.log(`    Language:   ${manifest.analysis?.language || '?'}`);
    console.log(`    Files:      ${manifest.analysis?.fileCount ?? '?'}`);
    console.log(`    Packages:   ${manifest.analysis?.packageCount ?? '?'}`);
    console.log(
      `    Knowledge:  ${manifest.knowledge?.documents ?? 0} docs, ${manifest.knowledge?.chunks ?? 0} chunks`,
    );
    console.log(`    Memory:     ${manifest.memory?.count ?? 0} events`);
    console.log();
  }
  console.log(`  ${BOLD}Verdict${RESET}`);
  if (allOk) console.log(`  ${GREEN}✓${RESET} All workspace storage paths verified.`);
  else {
    console.log(`  ${RED}✗${RESET} ${checks.filter((c) => !c.ok).length} issue(s) detected:`);
    for (const f of checks.filter((c) => !c.ok)) console.log(`    - ${f.label}${f.detail ? `: ${f.detail}` : ''}`);
    process.exitCode = 1;
  }
  console.log();
}
...
export async function runDoctorAll(): Promise<void> {
  console.log();
  console.log(`${BOLD}${GOLD}Full System Diagnostics${RESET}`);
  console.log(`${GRAY}─────────────────────────────────────${RESET}`);
  console.log();
  process.exitCode = 0;
  const sections: Array<{ name: string; fn: () => Promise<void> }> = [
    { name: 'General Health', fn: runDoctor },
    { name: 'Workspace', fn: runDoctorWorkspace },
    { name: 'Provider & Models', fn: runDoctorModels },
    { name: 'Conversation', fn: runDoctorConversation },
    { name: 'Audio', fn: runDoctorAudio },
    { name: 'Agents', fn: runDoctorAgents },
    { name: 'Teams', fn: runDoctorTeams },
  ];
  for (const section of sections) {
    ...
    try {
      ...
      console.log(`  ${RED}Error in ${section.name}: ${err.message}${RESET}\n`);
      process.exitCode = 1;
    }
  }
