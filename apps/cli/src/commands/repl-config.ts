...
export async function handleConfigSet(input: string, line: string, session: WorkspaceSession, rl: any): Promise<boolean> {
  ...
  if (!key || !value) { console.log(`${GRAY}  Usage: config set <key> <value>${RESET}`); rl.prompt(); return true; }
...
export async function handleConfigReset(input: string, line: string, session: WorkspaceSession, rl: any): Promise<boolean> {
  if (!input.startsWith('config reset ')) return false;
  const key = line.slice(13).trim();
  if (!key) { console.log(`${GRAY}  Usage: config reset <key>${RESET}`); rl.prompt(); return true; }
