...
export async function handleProjectsRoute(method: string, p: string, req: http.IncomingMessage, res: http.ServerResponse, ctx: WorkspaceContext): Promise<boolean> {
  if (method === 'GET' && p === '/api/projects') {
    if (!ctx.projects) { json(res, 200, { projects: [] }); return true; }
    const projects = await ctx.projects.listProjects();
    const withStats = await Promise.all(projects.map(async (p) => ({ ...p, stats: await ctx.projects!.getProjectStats(p.id) })));
    json(res, 200, { projects: withStats });
    return true;
  }

  if (method === 'GET' && p === '/api/sprints') {
    try { const allSprints = ctx.projects ? await ctx.projects.listSprints() : []; json(res, 200, { sprints: allSprints, active: allSprints.filter((s: any) => s.status === 'active') }); }
  ...
  if (method === 'GET' && projDetail) {
    const id = decodeURIComponent(projDetail[1]); const project = await ctx.projects?.getProject(id);
    if (!project) { json(res, 404, { error: 'project not found' }); return true; }
    json(res, 200, { project, tasks: await ctx.projects!.listTasks(id), sprints: await ctx.projects!.listSprints(id), stats: await ctx.projects!.getProjectStats(id) });
    return true;
  }
  ...
  if (method === 'POST' && p === '/api/projects') {
    ...
    const actor = getActor(req, ctx); const project = await ctx.projects?.createProject(body.name || 'New Project', body.description);
  ...
  if (method === 'PATCH' && taskDetail) {
    const id = decodeURIComponent(taskDetail[1]); const raw = await readBody(req); const body = raw ? JSON.parse(raw) : {};
  ...
  if (method === 'DELETE' && projDetail) {
    const id = decodeURIComponent(projDetail[1]); await ctx.projects?.updateProject(id, { status: 'cancelled' });
