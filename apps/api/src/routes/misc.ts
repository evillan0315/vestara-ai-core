...
import { json, readBody, getActor } from './types';

export async function handleMiscRoute(method: string, p: string, req: http.IncomingMessage, res: http.ServerResponse, ctx: WorkspaceContext, port: number, url: URL): Promise<boolean> {
  ...
  if (method === 'POST' && p === '/api/explain') {
    const raw = await readBody(req); const body = raw ? JSON.parse(raw) : {}; const target = body.target?.trim();
    if (!target) { json(res, 400, { error: 'target is required' }); return true; }
  ...
  if (method === 'POST' && p === '/api/analyze-feature') {
    ...
    if (!body.feature?.trim()) { json(res, 400, { error: 'feature is required' }); return true; }
  ...
  if (method === 'POST' && p === '/api/analyze-workspace') {
    try { json(res, 200, { analysis: await ctx.workspaceAnalyst.analyze(ctx.runtime.getSession()) }); }
  ...
  if (method === 'POST' && p === '/api/repl/execute') {
    ...
    const raw = await readBody(req); const body = raw ? JSON.parse(raw) : {}; const command = body.command?.trim();
    if (!command) { json(res, 400, { error: 'command is required' }); return true; }
    const lines: string[] = [];
    const fakeWs = { send: (s: string) => { try { const m = JSON.parse(s); if (m.op === 'output') lines.push(m.text); } catch {} } } as WebSocket;
  ...
  if (method === 'POST' && acceptMatch) {
    ...
    if (!suggestion) { json(res, 404, { error: 'suggestion not found' }); return true; }
    json(res, 200, { plan: (await ctx.planningService.createPlan(suggestion.title, ctx.runtime.getSession())).plan, suggestion: suggestion.id });
    return true;
  }
  ...
  if (method === 'POST' && dismissMatch) {
    ...
    await ctx.suggestionService.dismiss(suggestionId); await ctx.suggestionService.trackAction(suggestionId, 'dismissed');
