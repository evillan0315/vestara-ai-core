...
export async function handleTeamsRoute(
      ...
      const enriched = await Promise.all(
        teams.map(async (team) => {
          ...
          return {
            ...
            leader,
            stats: {
              totalExecutions: memberExecs.reduce((s, st) => s + st.total, 0),
              failedExecutions: memberExecs.reduce((s, st) => s + st.failed, 0),
  ...
  if (method === 'PUT' && teamMatch) {
    try {
      ...
      await ctx.agents.saveTeam({ ...existing, ...body, id });
      json(res, 200, { team: { ...existing, ...body, id } });
    } catch (err: any) {
      json(res, 500, { error: err.message });
    }
    return true;
