...
export async function handleSessionsRoute(method: string, p: string, req: http.IncomingMessage, res: http.ServerResponse, ctx: WorkspaceContext, port: number): Promise<boolean> {
  ...
  if (method === 'GET' && p === '/api/sessions/executions') {
    try { json(res, 200, { sessions: await ctx.agents.listExecutionSessions() }); } catch { json(res, 200, { sessions: [] }); }
    return true;
  }
  ...
  if (method === 'POST' && p === '/api/sessions/executions') {
    ...
    const session = { id: `exs-${Date.now()}-${Math.random().toString(36).slice(2, 4)}`, goal: body.goal || '', workflowId: body.workflowId, assignedAgentIds: body.assignedAgentIds || [], planIds: [], changeSetIds: [], verificationIds: [], logs: [], timeline: [], approvals: [], metrics: { duration: 0, totalSteps: 0, completedSteps: 0, artifactCount: 0 }, status: 'queued' as const, createdAt: now };
  ...
  if (method === 'GET' && exsMatch) {
    const id = decodeURIComponent(exsMatch[1]); const session = await ctx.agents.getExecutionSession(id);
    if (!session) { json(res, 404, { error: 'execution session not found' }); return true; }
    json(res, 200, { session });
    return true;
  }

  if (method === 'PATCH' && exsMatch) {
    const id = decodeURIComponent(exsMatch[1]); const raw = await readBody(req); const body = raw ? JSON.parse(raw) : {};
  ...
  if (method === 'POST' && p === '/api/sessions/executions/start') {
    const raw = await readBody(req); const body = raw ? JSON.parse(raw) : {};
    if (!body.goal || !body.workflow) { json(res, 400, { error: 'goal and workflow are required' }); return true; }
    try { json(res, 201, { session: await ctx.orchestrator.startSession(body.goal, body.workflow, ctx.runtime.getSession()) }); }
    catch (err: any) { json(res, 400, { error: err.message }); }
    return true;
  }

  if (method === 'POST' && p === '/api/background/run') {
    try { await ctx.orchestrator.runBackgroundServices(ctx.runtime.getSession()); json(res, 200, { ok: true }); }
